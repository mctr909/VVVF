    ...
    public partial class Form1 : Form {
        ...
        private void timer1_Tick(object sender, EventArgs e) {
            ...
            if (VvvfOut.EDisplayMode.PHASE == mWaveOut.DisplayMode) {
                ...
                for (int i = 0; i < mWaveOut.ScopeU.Length; i++) {
                    var x = center + (float)mWaveOut.ScopeU[i] * maxAmp;
                    var y = neutralLevel - (float)mWaveOut.ScopeV[i] * maxAmp;
                ...
                graph.DrawLine(Pens.Gray, 0, top, picWave.Width, top);
                graph.DrawLine(Pens.Gray, 0, bottom, picWave.Width, bottom);
                mScopeA = neutralLevel - (float)(mWaveOut.ScopeU[0] * maxAmp * scale);
                mScopeB = neutralLevel - (float)(mWaveOut.ScopeV[0] * maxAmp * scale);
                mScopeC = neutralLevel - (float)(mWaveOut.ScopeW[0] * maxAmp * scale);
                for (int i = 1, s = 0; s < picWave.Width; i += SCOPE_SPEED, s++) {
                    ...
                    for (int j = i; j < i + SCOPE_SPEED && j < mWaveOut.ScopeU.Length; j++) {
                        maxA = Math.Max(maxA, (float)mWaveOut.ScopeU[j]);
                        maxB = Math.Max(maxB, (float)mWaveOut.ScopeV[j]);
                        maxC = Math.Max(maxC, (float)mWaveOut.ScopeW[j]);
                        minA = Math.Min(minA, (float)mWaveOut.ScopeU[j]);
                        minB = Math.Min(minB, (float)mWaveOut.ScopeV[j]);
                        minC = Math.Min(minC, (float)mWaveOut.ScopeW[j]);
                    }
                    if (-minA < maxA) {
